...
/**
 * @extends AbstractUseCase<UploadProfilePictureCommand>
 */
class UploadProfilePictureUseCase extends AbstractUseCase
{
    ...
    {
        if (!$command instanceof UploadProfilePictureCommand) {
        ...
        if ($member->getProfilePicture()) {
            $oldFilePath = $this->uploadDirectory . '/profile-pictures/' . $member->getProfilePicture();
        ...
        $command->file->move(
            $this->uploadDirectory . '/profile-pictures',
            $fileName
        );
