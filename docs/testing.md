...
How to keep the backend test suite green, fast, and at ≥95% line coverage when
fixing bugs or building features. Read `backend/README.md` for the human-facing
summary; the [architecture](architecture.md) doc for the code patterns tests
exercise.
...
> and the php container must have `DATABASE_URL` set (from the compose
> `environment:` block — recreate the container with `docker compose up -d php`
> if `printenv DATABASE_URL` is empty).
